...
#define BLYNK_AUTH_TOKEN "KCTyVhBgVokaP0wzYfWADEfy4LvnA9Nh"
...
float clarity_min_pct = 40.0f;     // ambang minimal % dari clear_ref
...
//       String className = String(LR_CLASS_NAMES[cls]);
//       Serial.printf("[ML] Class=%s (%.0f%%)\n", className.c_str(), conf*100.0f);

//       // === Mapping ke STATUS1 sesuai permintaan ===
//       if (className == "MINYAK BERSIH" || className == "MINYAK BERSIH TEPUNG") {
//         lastResult = "LILIN";
//       } else if (className == "MINYAK KOTOR" || className == "MINYAK KOTOR TEPUNG") {
//         lastResult = "OIL SOLIDIFIER";
//       } else {
//         lastResult = "UNKNOWN"; // fallback
//       }

//       setUi(UI_SHOW_RESULT);  // update LCD
//       scanBusy = false;
//     }
//     vTaskDelay(pdMS_TO_TICKS(10));
//   }
...
void taskScan(void* pv) {
  ...
  for (;;) {
    if (scanRequested && !scanBusy) {
      ...
      // ===== RULE GABUNGAN =====
      if (className == "MINYAK BERSIH" || className == "MINYAK BERSIH TEPUNG") {
        // ML dominan → selalu LILIN
        lastResult = "LILIN";
      } else if (className == "MINYAK KOTOR" || className == "MINYAK KOTOR TEPUNG") {
        // Kalau kotor, cek clarity
        if (clarity_pct >= 0 && clarity_pct >= clarity_min_pct) {
          lastResult = "LILIN";          // masih bisa diolah
        } else {
          lastResult = "OIL SOLIDIFIER"; // benar2 kotor
        }
      } else {
        lastResult = "UNKNOWN"; // fallback
      }
      ...
      // ===== DEBUG SERIAL =====
      // Serial.printf("[ML] Class=%s (%.0f%%), C=%u, Clarity=%.1f%% (ambang %.1f), T=%.1fC, Berat=%d mL -> Result=%s\n",
      //   className.c_str(), conf*100.0f, lastCraw, clarity_pct, clarity_min_pct,
      //   isnan(senSuhu)?0.0:senSuhu, beratMinyak, lastResult.c_str());
      Serial.printf("[ML] Class=%s (%.0f%%), C=%u, Clarity=%.1f%%, T=%.1fC, Berat=%d mL\n",
        LR_CLASS_NAMES[cls], conf*100.0f, lastCraw, clarity_pct, senSuhu, beratMinyak);
